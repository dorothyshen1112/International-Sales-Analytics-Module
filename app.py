...
def smart_normalize(df):
    ...
    # 過濾年份雜訊 (保留正常區間)
    p_df = p_df[(p_df['年度'] > 2010) & (p_df['年度'] < 2030)].copy()
...
if df_all is not None:
    ...
    if not df.empty:
        ...
        with tabs[0]: # YoY 成長表
            ...
            with c1: st.plotly_chart(px.pie(df.groupby('國家')[metric_opt].sum().reset_index(), values=metric_opt, names='國家', hole=0.4, title="各國佔比", color_discrete_sequence=px.colors.qualitative.Pastel), use_container_width=True)
            with c2:
                ...
                def color_logic(v):
                    if pd.isna(v): return 'color: #9E9E9E' # None 顯示為灰色
                    return f'color: {"#D32F2F" if v > 0.001 else "#388E3C"}; font-weight: bold' # 紅漲綠跌

                st.write("#### 逐年成長趨勢表 (YoY%)")
                # 強制應用格式化與染色
                styled = disp[f_cols].style.format(f_dict, na_rep="-").applymap(color_logic, subset=[c for c in f_cols if '成長%' in c])
