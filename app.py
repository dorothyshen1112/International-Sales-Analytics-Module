...
def smart_normalize(df):
    ...
    if bx_note_series is not None:
        ...
        p_df['備註'] = df[find_best_col(['備註', '備註1', 'REMARK', '說明'])].loc[p_df.index].astype(str).replace('nan', '') if find_best_col(['備註', '備註1', 'REMARK', '說明']) else ""
    ...
    # 關鍵：強制初始化所有 FOC 類別欄位為 0，防止 KeyError
    for cat in foc_cats:
...
if uploaded_file:
    ...
    if not df.empty:
        ...
        with tabs[3]: # 營運效率
            ...
            with c_eff1:
                ...
                st.plotly_chart(px.bar(active_cust.sort_values('客戶', ascending=False), x='國家', y='客戶', color='商務模式', title="活躍客戶總數(滲透力)"), use_container_width=True)
            with c_eff2:
                ...
                st.plotly_chart(px.bar(order_size.sort_values('平均單次採購量', ascending=False), x='國家', y='平均單次採購量', color='商務模式', title="平均單筆訂單規模"), use_container_width=True)
    ...
    st.info("👋 管理員您好，請導入 Excel。本版已全面修正 FOC 分類報錯與年份顯示問題。")
