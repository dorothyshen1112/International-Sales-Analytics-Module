...
def smart_normalize(df):
    ...
    def find_col(key):
        for name in m.get(key, []):
            # 增加模糊匹配，只要包含關鍵字且長度相近就認列
            for real_col in df.columns:
                if name in real_col: return real_col
        return None

    p_df = pd.DataFrame()
    
    # 智慧時間識別：優先找年度數字 (解鎖 2007-2022)
    ...
    if not df.empty:
        ...
        with tabs[3]: # 營運效率 (藍色說明永久鎖定)
            ...
            st.markdown("""<div class="report-note">
                ...
                ● <b>意義：</b> 日本(直營)顯示已進入診所總數；經銷商國家應為 1。<br>
                ● <b>分析重點：</b> 日本數值越高代表開拓力越強；經銷商若變為0則有市場停滯風險。
            ...
            st.markdown("""<div class="report-note">
                <b>2. 物流模式分析 (平均單次規模)：</b><br>
                ● <b>意義：</b> 經銷商大宗採購(高) vs 日本診所小量多次(低)。<br>
                ● <b>分析重點：</b> 若經銷商數值低於 100 支，代表報關頻率過高，物流成本過重。
            </div>""", unsafe_allow_html=True)
            ...
            st.markdown("""<div class="report-note">
                ...
                ● <b>意義：</b> 每投入1支贈針換回幾張訂單。數值越高代表投資報酬率越高。<br>
                ● <b>分析重點：</b> 評估 Workshop、Demo 的變現轉單力。
            </div>""", unsafe_allow_html=True)
