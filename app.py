...
def smart_normalize(df):
    ...
    m = {
        ...
        'AMOUNT': ['本幣未稅金額', '本幣合計', '金額', '未稅金額', 'AMOUNT', '總計', '本幣金額', '銷貨金額', '未稅金額_1']
    }
...
if df_all is not None:
    ...
    if not df.empty:
        ...
        with tabs[0]: 
            ...
            st.plotly_chart(px.pie(df.groupby('國家')[m_col].sum().reset_index(), values=m_col, names='國家', hole=0.4, title=f"各國份額佔比 ({yr_str})", color_discrete_sequence=px.colors.qualitative.Pastel), use_container_width=True)
        ...
        with tabs[3]: # 營運效率 (藍色說明框永久固定)
            ...
            with ce2:
                st.markdown('<div class="report-note"><b>2. 物流模式分析：</b><br>● 經銷商大宗採購 vs 日本診所小量多次。</div>', unsafe_allow_html=True)
