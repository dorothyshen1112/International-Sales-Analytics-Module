...
def smart_normalize(df):
    ...
    m = {
        ...
        'YEAR_ONLY': ['年度', '年', 'YEAR', '年度_1', '年份', '西元年', 'YYYY'],
        'COUNTRY': ['國家', '地區', 'COUNTRY', '區域', '廠別名稱', '地  區'],
        'CUSTOMER': ['客戶簡稱', '客戶', '客戶名稱', 'CUSTOMER', '送貨客戶全名', '客戶全名'],
        'PRODUCT': ['品名', '業務品名', '產品名稱', 'PRODUCT', '品號', '業務規格', '品  名'],
        'QTY': ['銷貨數量', '數量', '計價數量', '總數量', 'QTY', '件數', '銷貨數量_1'],
        'GIFT_QTY': ['贈/備品量', '贈品量', '贈品數量', 'GIFTQTY'],
        'PRICE': ['單價', '單價NT', 'PRICE', '單   價', '單  價'],
        'AMOUNT': ['本幣未稅金額', '本幣合計', '金額', '未稅金額', 'AMOUNT', '總計', '本幣金額', '銷貨金額', '銷貨金額_1', '幣別金額']
    }
    ...
    if c_date:
        ...
        # 如果真的都沒有日期，給一個預設值避免該 Sheet 消失
        p_df['年度'] = 2000
        p_df['銷貨日期'] = pd.to_datetime('2000-01-01')
...
if df_all is not None:
    ...
    # --- 關鍵修正：預設全選所有年份，確保 2013 等年份自動跑出來 ---
