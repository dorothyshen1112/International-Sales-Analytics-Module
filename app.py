...
st.markdown("""
    ...
    .stDataFrame { border: 1px solid #F1F3F5; }
    </style>
    """, unsafe_allow_html=True)

# --- 2. 智慧資料正規化引擎 (處理新舊系統與多Sheet) ---
def smart_normalize(df):
    if df.empty: return pd.DataFrame()
    
    # [特殊邏輯] 優先抓取 BX 欄位 (新系統第76欄) 作為詳細備註
    bx_note = None
    if df.shape[1] >= 76:
    ...
    # 產品合併邏輯：將 DeusaDerm 合併至 Deusaderm VITAL
...
with st.sidebar:
    ...
    if admin_mode:
        ...
        if pwd == "sunmax888":
            uploaded_file = st.file_uploader("請上傳包含新舊系統工作表的 Excel", type=["xlsx"])
...
if uploaded_file:
    ...
    # --- 全域置頂篩選器 ---
    st.info("💡 系統已自動整合跨系統、跨年度數據。請設定下方條件以更新分析圖表：")
    sc1, sc2, sc3 = st.columns([1, 1, 2])
    sel_area = sc1.multiselect("區域", ['台灣市場', '海外市場'], default=['海外市場'])
    ...
    sel_countries = sc3.multiselect("國家", countries, default=countries)
    ...
    if not df.empty:
        ...
        k4.metric("平均贈針比", f"{(df['FOC總量'].sum()/(df['數量'].sum()+0.0001)*100):.1%}%")
        ...
        with tabs[0]: 
            ...
            for i in range(1, len(sorted_cols)):
                ...
                display_df[f"{curr} 成長率%"] = ((pivot_df[curr] - pivot_df[prev]) / (pivot_df[prev] + 0.001) * 100).replace([np.inf, -np.inf], 0)
            
            # 欄位排序
            ordered = []
            for y in sorted_cols:
                ordered.append(y)
                if f"{y} 成長率%" in display_df.columns: ordered.append(f"{y} 成長率%")
            
            def style_growth(val, col):
                return f'color: {"green" if val > 0 else "red" if val < 0 else "black"}; font-weight: bold' if '成長率' in str(col) else ''
        ...
        with tabs[1]: # 產品分析
            ...
            st.plotly_chart(px.bar(p_df, x='金額', y='產品', orientation='h', color='金額', color_continuous_scale='Blues', title="熱銷產品 Top 12"), use_container_width=True)

        with tabs[2]: # FOC 專項
            st.subheader(f"📉 FOC 原因分佈與明細 ({yr_str})")
        ...
        with tabs[3]: # 營運效率
            ...
            ec2.plotly_chart(px.bar(logi_df.sort_values('單筆採購量', ascending=False), x='國家', y='單筆採購量', title="物流效率 (平均每單件數)", color='單筆採購量', color_continuous_scale='Greens'), use_container_width=True)

        with tabs[4]: # 數據明細
            search_q = st.text_input("輸入關鍵字搜尋客戶、產品或備註...").lower()
            q_df = df.copy()
            if search_q:
    ...
    st.info("👋 管理員您好，請上傳 Excel 檔案。系統將自動分析所有工作表 (Sheet)，並自動適配新舊系統格式。")
