...
def smart_normalize(df):
    ...
    # 沒對到關鍵字但單價為0的，補入「培訓與實操用針」(作為預設)
    p_df['FOC總量'] = np.where((p_df['本行FOC總量'] > 0) & (p_df['FOC總量'] == 0), p_df['本行FOC總量'], p_df['FOC總量'])
    p_df.loc[(p_df['本行FOC總量'] > 0) & (p_df[list(kw.keys())].sum(axis=1) == 0), '培訓與實操用針'] = p_df['本行FOC總量']
...
if uploaded_file:
    ...
    if not df.empty:
        ...
        # 總分母 = 收費數量 + 贈針數量
        total_qty_denom = df['數量'].sum() + df['贈品量_欄位'].sum()
        foc_rate = (df['FOC總量'].sum() / (total_qty_denom + 0.0001) * 100)
        ...
        with tabs[2]: # FOC 專項
            ...
            st.write("#### FOC 識別明細 (含贈品量欄位與單價0項目)")
        ...
        with tabs[3]: # 營運效率
            ...
            ec1.plotly_chart(px.bar(eff_df.sort_values('轉換率', ascending=False), x='國家', y='轉換率', title="FOC 投資轉換率 (每贈1支換回幾單)", color='轉換率'), use_container_width=True)
    ...
    st.info("👋 管理員您好，請導入 Excel 檔案。系統會自動處理多工作表並修正 FOC 數量。")
