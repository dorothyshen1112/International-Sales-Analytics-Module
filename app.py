...
def smart_normalize(df):
    ...
    m = {
        ...
        'AMOUNT': ['本幣未稅金額', '本幣合計', '金額', '未稅金額', 'AMOUNT', '總計', '本幣金額', '銷貨金額', '幣別金額', '合計', '銷貨淨額']
    }
    ...
    if not df.empty:
        ...
        with tabs[0]: 
            ...
            for y in sorted_y:
                ...
                col_cfg[y_c] = st.column_config.NumberColumn(width=200, format="%d")
                g_c = f"{int(y)}年 成長%"
                if g_c in disp.columns:
                    final_f_cols.append(g_c)
                    col_cfg[g_c] = st.column_config.TextColumn(width=200)

            def sty_rg(v, n): return f'color: {"#D32F2F" if v>0.001 else "#388E3C"}; font-weight: bold' if '成長%' in str(n) and pd.notna(v) else ''
            # --- 關鍵修正：強制大格顯示且不自動壓縮寬度 ---
            st.dataframe(disp[final_f_cols].style.format({c: "{:.1f}%" if '成長%' in c else "{:,.0f}" for c in final_f_cols}, na_rep="-").apply(lambda x: [sty_rg(v, x.name) for v in x], axis=0), height=500, use_container_width=False, column_config=col_cfg)

        with tabs[3]: # 營運效率 (說明框永久固定)
            ...
            st.divider()
            st.markdown('<div class="report-note"><b>2. 物流模式分析：</b><br>● 經銷商大宗進貨數值高；日本診所小量多次。</div>', unsafe_allow_html=True)
        ...
        with tabs[2]: # FOC (修正變數名稱錯誤)
            ...
            f_data_view = df[df['FOC總量']>0].copy() # 修正變數名稱
