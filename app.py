...
# --- 3. 數據加載邏輯 (智慧持久化) ---
@st.cache_data(show_spinner="正在讀取全球數據...")
def load_data(file_source):
    all_sheets = pd.read_excel(file_source, sheet_name=None)
...
with st.sidebar:
    ...
    if admin_mode:
        ...
        if pwd == "sunmax888":
            uploaded_file = st.file_uploader("上傳最新 Excel (臨時更新)", type=["xlsx"])
            st.info("註：若要永久更新，請將檔案上傳至 GitHub 並更名為 data.xlsx")

# 決定數據來源
final_df_all = None
if uploaded_file:
    final_df_all = load_data(uploaded_file)
elif os.path.exists("data.xlsx"):
    final_df_all = load_data("data.xlsx")

st.title("📊 双美全球銷售數據")

if final_df_all is not None:
    df_all = final_df_all
    ...
    if not df.empty:
        ...
        with tabs[3]: # 營運效率
            ...
            with c_eff1:
                st.markdown('<div class="report-note"><b>1. 市場滲透度分析：</b><br>顯示活躍客戶/診所數。</div>', unsafe_allow_html=True)
            ...
            with c_eff2:
                st.markdown('<div class="report-note"><b>2. 物流採購規模：</b><br>顯示平均單次採購規模。</div>', unsafe_allow_html=True)
                ...
                st.plotly_chart(px.bar(order_size.sort_values('平均單次採購量', ascending=False), x='國家', y='平均單次採購量', color='商務模式', title="物流模式", text_auto=True), use_container_width=True)
            
            eff_df = df.groupby('國家').agg({'收費量':'sum', 'FOC總量':'sum'}).reset_index()
            eff_df['行銷效率'] = (eff_df['收費量'] / (eff_df['FOC總量'] + 0.001)).round(1)
            st.plotly_chart(px.bar(eff_df.sort_values('行銷效率', ascending=False), x='國家', y='行銷效率', title="3. 行銷投資回報 (1支贈針換回幾支訂單)", color='行銷效率', text_auto=True), use_container_width=True)
    ...
    st.warning("👋 尚未發現數據檔案。管理員請將 Excel 命名為 data.xlsx 並上傳至 GitHub，或從左側管理員模式手動上傳。")
