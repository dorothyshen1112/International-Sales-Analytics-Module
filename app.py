...
def smart_normalize(df):
    ...
    # 過濾年份雜訊 (保留 2010-2030)
    p_df = p_df[(p_df['年度'] > 2010) & (p_df['年度'] < 2030)]
    ...
    raw_cust = df[c_cust].astype(str) if c_cust else pd.Series(['未知客戶']*len(p_df))
    ...
    p_df['產品'] = df[c_prod].astype(str).str.replace('Sunmax DeusaDerm', 'Sunmax Deusaderm VITAL', case=False).str.replace('VITAL VITAL', 'VITAL', case=False) if c_prod else "未知產品"
    
    def to_num(key):
        col = get_col(key)
        return pd.to_numeric(df[col].astype(str).str.replace(',', ''), errors='coerce').fillna(0.0) if col else pd.Series([0.0]*len(df))
    ...
    f_cats = ['培訓活動用針', '醫師酬勞', 'Workshop Training', 'Training/Rebate for JP', '市場贊助', '研究用針', '客訴補償', 'FOC樣品運輸']
    for cat in f_cats: p_df[cat] = 0.0
...
def load_data(file):
    try:
        ...
        return pd.concat([smart_normalize(df_t) for n, df_t in sheets.items() if not df_t.empty], ignore_index=True)
...
final_df = None
if uploaded_file: final_df = load_data(uploaded_file)
elif os.path.exists("data.xlsx"): final_df = load_data("data.xlsx")
...
if final_df is not None:
    ...
    sel_yrs = sc2.multiselect("年度", options=available_yrs, default=available_yrs)
    ...
    if not df.empty:
        ...
        tot = df['收費量'].sum() + df['FOC總量'].sum()
        k4.metric("平均贈針比", f"{(df['FOC總量'].sum()/(tot+0.0001)*100):.1f}%")
        ...
        with tabs[0]: # YoY 成長
            ...
            with c1: st.plotly_chart(px.pie(df.groupby('國家')[metric_opt].sum().reset_index(), values=metric_opt, names='國家', hole=0.4, title="各國份額", color_discrete_sequence=px.colors.qualitative.Pastel), use_container_width=True)
            with c2:
                pivot = df.pivot_table(index='國家', columns='年度', values=metric_opt, aggfunc='sum').fillna(0)
                ...
                st.dataframe(disp[f_cols].style.format(f_dict, na_rep="-").apply(lambda x: [color_logic(v, x.name) for v in x], axis=0), use_container_width=True)

        with tabs[3]: # 營運效率 (藍色框回歸)
            ...
            with ce2:
                st.markdown('<div class="report-note"><b>2. 物流模式分析：</b><br>● 數值越高代表單次採購量大，物流效率越好。</div>', unsafe_allow_html=True)
